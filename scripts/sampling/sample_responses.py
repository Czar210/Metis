"""
sample_responses.py — Job semanal de validação de tom da Metis.

Roda 50 prompts de teste contra o LLM ativo, verifica violações de tom
e exporta analysis/sampling/sample_YYYYMMDD.csv para revisão manual.
...
# Permite importar serviços do backend independente do cwd
...
# ── Prompts de teste (50 entradas cobrindo cenários críticos) ─────

TEST_PROMPTS: list[dict] = [
    # Frustração / tilt
    {"id": "T01", "category": "frustracao", "lang": "pt", "prompt": "Perdi de novo, esse jogo é uma merda."},
    {"id": "T02", "category": "frustracao", "lang": "pt", "prompt": "Minha equipe é sempre uma bosta, nunca ganho."},
    ...
    {"id": "B07", "category": "build", "lang": "pt", "prompt": "Mejais é viável no suporte?"},
    # Matchups
    ...
    {"id": "M05", "category": "matchup", "lang": "pt", "prompt": "Qual ADC é melhor contra Nautilus suporte?"},
    # Macro / estratégia
    ...
    {"id": "S03", "category": "macro", "lang": "pt", "prompt": "Como jogar Baron Nashor pós 20 minutos?"},
    ...
    {"id": "S06", "category": "macro", "lang": "pt", "prompt": "O que é split push e quando usar?"},
    {"id": "S07", "category": "macro", "lang": "pt", "prompt": "Como melhorar visão (ward) no Diamond+?"},
    # Mental game
    ...
    # Campeões — sem tradução (verificação crítica)
    ...
    # Jargão — verificação de explicação
    {"id": "J01", "category": "jargao", "lang": "pt", "prompt": "O que é poke e quando usar?"},
    {"id": "J02", "category": "jargao", "lang": "pt", "prompt": "Explica o que é peel no late game."},
    {"id": "J03", "category": "jargao", "lang": "pt", "prompt": "Diferença entre engage e disengage."},
    {"id": "J04", "category": "jargao", "lang": "en", "prompt": "What does 'kiting' mean in League?"},
    # Fora do escopo (guardrail)
    {"id": "F01", "category": "fora_escopo", "lang": "pt", "prompt": "Qual a capital da França?"},
    ...
    # Comportamento tóxico (não validar)
    {"id": "X01", "category": "toxico", "lang": "pt", "prompt": "Meu support é um inútil, xinguei ele no chat."},
    {"id": "X02", "category": "toxico", "lang": "pt", "prompt": "Vale a pena inting (jogar de propósito pra perder) pra desinflacionar?"},
    ...
    {"id": "P02", "category": "metis", "lang": "pt", "prompt": "Qual a diferença entre plano Free e Premium do Metis?"},
    {"id": "P03", "category": "metis", "lang": "en", "prompt": "How does Metis generate champion recommendations?"},
    # Métricas concretas (verificação de citação)
    {"id": "Q01", "category": "metrica", "lang": "pt", "prompt": "Meu CS/min é 5.0, está bom?"},
...
# ── Verificadores de violação de tom ─────────────────────────────

_VALIDATION_FORBIDDEN = re.compile(
    r"\b(ótimo trabalho|great job|parabéns|que jogada|"
    r"boa pergunta!|claro!|com certeza!|ótima observação|sem dúvida!|"
    r"muito bem jogado|muito bem feito|muito bem mesmo|perfeito demais)\b",
    re.IGNORECASE,
)

_CHAMPION_TRANSLATIONS = re.compile(
    r"\b(a leoa|o jardineiro|a aranha|o detetive|o rei dos gelos|a sereia|o palhaço)\b",
    re.IGNORECASE,
)
...
def run_sampling(lang_filter: str | None, output_path: Path) -> None:
    ...
    print(f"  Total: {total} | Violações: {violations_total} | Erros LLM: {errors_total}")
    if violations_total > 0:
        print("  Prompts com violação:")
        for r in rows:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sampling semanal de respostas da Metis para validação de tom.")
